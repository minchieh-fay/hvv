...
export function createPlanSegmentsTool(onPlan = null) {
  return tool({
    ...
    execute: async ({ scenes }) => {
      const segments = help_buildSegmentPlan(scenes);
      onPlan?.(segments);
      return JSON.stringify({ success: true, frameRate: 24, maxSeconds: 5, segments });
    },
  });
}
