...
async function agentGenerateKeyframe(input) {
  const task = keyframeQueue.catch(() => {}).then(async () => {
...
function extractReferencePath(value) {
  ...
  const path = matched?.[1] || text;
  if (path.startsWith("data:image/") || path.includes("/")) return path;
  const segmentID = path.match(/segment-\d+/)?.[0];
  if (segmentID && path.includes("first")) return `segments/${segmentID}/first.png`;
  if (segmentID && path.includes("last")) return `segments/${segmentID}/last.png`;
  return path;
}
...
async function agentGenerateSegment(input) {
  const existingTask = segmentTasks.get(input.segmentID);
  if (existingTask) return existingTask;
  const task = generateSegmentOnce(input);
  segmentTasks.set(input.segmentID, task);
  return task;
}

// 保存 Agent 规划出的总片段数，用于展示用户可读的总体进度。
function handlePlanSegments(plannedSegments) {
  plannedSegmentCount.value = plannedSegments.length;
}

// 将内部片段编号转换成用户可读的进度文本。
function formatSegmentProgress(segmentID, progress) {
  const matched = String(segmentID).match(/(\d+)$/);
  const current = matched ? Number(matched[1]) : 0;
  const total = plannedSegmentCount.value || current || 1;
  return `第${current || "当前"}块(${progress}%), 总进度${current || 1}/${total}`;
}

// 压缩图片响应中的 Base64 正文，保留响应元数据和内容长度供问题回溯。
function compactAgnesImageResponse(response) {
  return {
    ...response,
    data: response.data?.map((item) => {
      const { b64_json, ...metadata } = item;
      return { ...metadata, b64_jsonLength: b64_json?.length || 0 };
    }),
  };
}

// 创建单个视频片段任务，并等待原任务完成后保存结果。
async function generateSegmentOnce(input) {
  const context = await loadVideoContext();
  ...
  await recordEvent("video.requested", {
    ...
    prompt: input.prompt,
    frames: [input.firstFrame, input.lastFrame],
    frameURLs: [input.firstFrame, input.lastFrame].map((frame) =>
      buildPublicMediaURL(context.publicURL, frame),
    ),
    numFrames: input.numFrames,
    taskID,
    statusURL: `${rootURL}/agnesapi?video_id=${encodeURIComponent(taskID)}`,
    response: result,
    model: "agnes-video-v2.0",
  });
  const completed = await waitForAgnesVideo(result, context, new AbortController().signal, (progress) => {
    progressText.value = formatSegmentProgress(input.segmentID, progress);
  }, async (response, httpStatus) => {
    await recordEvent("video.status", {
      segmentID: input.segmentID,
      taskID,
      statusURL: `${rootURL}/agnesapi?video_id=${encodeURIComponent(taskID)}`,
      httpStatus,
  ...
  const agnesVideoURL = completed.metadata?.url || completed.url;
  await recordEvent("video.result", {
    segmentID: input.segmentID,
    taskID,
    agnesVideoURL,
  ...
  const videoResponse = await fetch(agnesVideoURL);
  ...
  await recordEvent("video.downloaded", {
    segmentID: input.segmentID,
    taskID,
    agnesVideoURL,
    path: videoPath.path,
    url: videoURL,
    publicURL: buildPublicMediaURL(context.publicURL, videoPath.path),
    size: videoBlob.size,
  });
...
async function agentConcat(blob) {
  ...
  await recordEvent("video.finalized", {
    path: saved.path,
    url: `/media/${saved.path}`,
    publicURL: buildPublicMediaURL((await loadVideoContext()).publicURL, saved.path),
    size: blob.size,
  });
...
async function generateKeyframe(prompt, referencePaths, path) {
  try {
    ...
    await recordEvent("keyframe.requested", {
      ...
      referenceURLs: referencePaths.map((reference) => buildPublicMediaURL(context.publicURL, reference)),
      output: path,
      outputURL: buildPublicMediaURL(
        context.publicURL,
        `${session.value.date}/video/${session.value.id}/${path}`,
      ),
      model: "agnes-image-2.1-flash",
    });
    const dataURL = await callAgnesImage(
      { prompt, references: referencePaths, ratio: ratio.value },
      context,
      undefined,
      async (response, httpStatus) => {
        await recordEvent("image.response", {
          httpStatus,
          response: compactAgnesImageResponse(response),
        });
      },
    );
    ...
    await recordEvent("keyframe.completed", {
      path: saved.path,
      url: `/media/${saved.path}`,
      publicURL: buildPublicMediaURL(context.publicURL, saved.path),
      size: blob.size,
    });
    return saved.path;
  } catch (error) {
    lastVideoToolError = error;
...
async function generateVideo() {
  ...
  generating.value = true;
  lastVideoToolError = null;
  keyframeQueue = Promise.resolve();
  segmentTasks = new Map();
  plannedSegmentCount.value = 0;
  segments.value = [];
  ...
  try {
    ...
    const { agent, runner } = createVideoProductionAgent(context, {
      ratio: ratio.value,
      planSegments: handlePlanSegments,
    ...
    if (!segments.value.some((segment) => segment.url && Number(segment.actualDuration) > 0)) {
      throw lastVideoToolError || new Error("视频生成未产生可播放片段，请检查 Agent 日志中的首帧或视频接口错误");
    }
...
async function finalizeVideo() {
  ...
  try {
    ...
    await recordEvent("video.finalized", {
      ...
      publicURL: buildPublicMediaURL(context.publicURL, saved.path),
      size: blob.size,
      segments: timeline.value.map((item) => item.id),
    });
