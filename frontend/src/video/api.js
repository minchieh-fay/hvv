...
// 将本地媒体图片读取为 Data URI，避免图片模型读取刚生成的公网图片失败。
async function loadImageReferenceData(path) {
  if (path.startsWith("data:image/")) return path;
  const response = await fetch(`/media/${path}`);
  if (!response.ok) throw new Error(`读取参考图失败：${path}`);
  const blob = await response.blob();
  return await new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve(reader.result);
    reader.onerror = () => reject(new Error(`参考图转换失败：${path}`));
    reader.readAsDataURL(blob);
  });
}

// 调用 Agnes Image 2.1 Flash 生成视频关键帧图片。
export async function callAgnesImage(payload, context, signal) {
  const references = payload.references || [];
  const imageInputs = await Promise.all(references.map(loadImageReferenceData));
  const requestBody = {
    ...
    extra_body: {
      ...
      ...(references.length ? { image: imageInputs } : {}),
    },
  };
...
export async function waitForAgnesVideo(task, context, signal, onProgress = () => {}) {
  ...
  for (;;) {
    ...
    const result = await response.json();
    if (!response.ok) throw new Error(describeVideoError(result.error, "查询视频任务失败"));
